...
async function getSharp() {
  try {
    ...
    console.log('sharp module not found in node_modules, trying to dynamically install...');
    try {
      ...
      console.warn('Could not load or install sharp:', installErr.message);
      return null;
    }
  }
}
...
async function generate() {
  console.log('Starting Meridian App Icon Generation...');
  ...
  if (!sharp) {
    console.warn('sharp is unavailable. Checking existing generated icon assets in public/...');
    if (fs.existsSync('public/icon-192.png') && fs.existsSync('public/icon-512.png')) {
      console.log('Found existing generated icons. Skipping sharp rendering.');
      return;
    }
    console.error('No sharp library and missing icons. Please run "npm install" first.');
    process.exit(1);
  }

  // Standard sizes for PWA / Android
  ...
  // Also update Android native mipmap folders if present
  const androidMipmaps = [
    { dir: 'app/src/main/res/mipmap-mdpi', size: 48 },
    { dir: 'app/src/main/res/mipmap-hdpi', size: 72 },
    { dir: 'app/src/main/res/mipmap-xhdpi', size: 96 },
    { dir: 'app/src/main/res/mipmap-xxhdpi', size: 144 },
    { dir: 'app/src/main/res/mipmap-xxxhdpi', size: 192 }
  ];
  ...
  for (const { dir, size } of androidMipmaps) {
    ...
    if (fs.existsSync(fullDir)) {
      ...
      console.log(`Updated Android launcher icons in ${dir} (${size}x${size})`);
    }
  }
